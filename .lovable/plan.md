...
A regra no banco já existe e adiciona as coordenadoras/assistentes como envolvidas — mas só quando o item é gravado com coordenação preenchida.

- Quem tem apenas uma coordenação continua como está hoje: sem select, vinculação automática à própria coordenação.
- O coordenador da coordenação entra automaticamente como envolvido, exibido com o cadeado (não removível), no mesmo padrão já usado nos formulários.
- Sem duplicar quando já estiver na lista e sem substituir os responsáveis. Quem cria já sendo coordenador não entra duas vezes.

## 4. Etiquetas de cliente

Fora deste escopo por enquanto — você vai alinhar com a advogada antes.
...
- `src/components/delegacao/NovaTarefaDialog.tsx`, `PrazoDialog.tsx`, `EventoDialog.tsx`, `AudienciaFormSimplificado.tsx`: Data base inicial derivada de `publicacao.data_disponibilizacao ?? publicacao.data_publicacao`.
- Envolvidos: `useCoordenacoesDoUsuario` continua definindo `unicaCoordenacaoId` (sem select); `useCoordenadoresDaCoordenacao` alimenta o envolvido fixo com cadeado, e a gravação usa o `ON CONFLICT DO NOTHING` já existente nas tabelas de envolvidos.

## Sequência

1. Correção da leitura (item 1) — maior impacto.
2. Data base e coordenadores envolvidos (itens 2 e 3).
3. Revisão do "Desfazer último" junto da correção de leitura.