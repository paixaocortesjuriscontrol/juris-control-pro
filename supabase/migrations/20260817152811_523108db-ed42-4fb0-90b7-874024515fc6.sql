DROP FUNCTION IF EXISTS public.get_ranking_atendimento_geral(date,date,uuid,uuid);
...
tar AS (
 SELECT b.*,
  ...
  -- Prazo confiável para medir pontualidade: em cargas importadas, a data de vencimento
  -- costuma ser a própria data de conclusão da planilha, então só vale data fatal/prevista.
  CASE WHEN b.origem IN ('astrea','projuris','importacao','import','planilha','migracao','carga','benner')
       THEN coalesce(b.data_fatal,b.data_prevista)
       ELSE coalesce(b.data_fatal,b.prazo) END AS prazo_ref,
...
concl AS (
 SELECT uid,count(*) c_total,
  count(*) FILTER(WHERE prazo_ref IS NOT NULL) c_aval,
  count(*) FILTER(WHERE prazo_ref IS NOT NULL AND data_conclusao_filtro::date<=prazo_ref) c_prazo,
  count(*) FILTER(WHERE prazo_ref IS NOT NULL AND data_conclusao_filtro::date>prazo_ref) c_atraso
 FROM tar_resp
...
perdidos AS (
 SELECT uid,count(*) p_total FROM tar_resp WHERE prazo_ref IS NOT NULL AND prazo_ref BETWEEN p_inicio AND p_fim AND ((status IN ('cumprido','tratado','protocolado','baixado','verificado') AND data_conclusao_filtro IS NOT NULL AND data_conclusao_filtro::date>prazo_ref) OR (status NOT IN ('cumprido','tratado','protocolado','baixado','verificado','cancelado','concluido_sem_sucesso') AND prazo_ref<current_date)) GROUP BY uid
),
ativ_base AS (
...
SELECT i.uid,coalesce(p.nome,'Sem nome'),coalesce(a.a_prazos,0),coalesce(a.a_tarefas,0),coalesce(a.a_audiencias,0),coalesce(a.a_eventos,0),coalesce(a.a_parcelamentos,0),coalesce(a.a_total,0),coalesce(c.c_total,0),coalesce(c.c_prazo,0),coalesce(c.c_atraso,0),coalesce(pd.p_total,0),coalesce(av.at_total,0),coalesce(av.at_concl,0),coalesce(c.c_aval,0)
