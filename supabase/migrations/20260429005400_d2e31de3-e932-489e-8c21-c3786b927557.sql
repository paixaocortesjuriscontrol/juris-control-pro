...
DECLARE
  ...
  v_lidas_t bigint := 0;
  v_lidas_p bigint := 0;
  v_filtra_termos boolean;
  v_filtra_processos boolean;
BEGIN
  v_uid := auth.uid();
  ...
  v_filtra_termos    := (v_tipo IS NULL OR v_tipo IN ('termo', 'parte'));
  v_filtra_processos := (v_tipo IS NULL OR v_tipo = 'processo') AND p_monitoramento_id IS NULL;

  -- =========================================================================
  -- TERMOS
  -- =========================================================================
  IF v_filtra_termos THEN
    SELECT COUNT(DISTINCT pd.dedup_key)
      INTO v_total_t
    ...
    SELECT COUNT(DISTINCT pd.dedup_key)
      INTO v_lidas_t
    FROM public.publicacoes_djen pd
    JOIN public.publicacoes_djen_leituras l
      ON l.publicacao_id = pd.id
  ...
  -- PROCESSOS
  -- =========================================================================
  IF v_filtra_processos THEN
    SELECT COUNT(DISTINCT pdp.dedup_key)
      INTO v_total_p
  ...
  total_termos        := v_total_t;
  total_processos     := v_total_p;
  nao_lidas_termos    := GREATEST(0, v_total_t - v_lidas_t);
  nao_lidas_processos := GREATEST(0, v_total_p - v_lidas_p);
