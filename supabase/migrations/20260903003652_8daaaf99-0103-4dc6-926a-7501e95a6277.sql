...
BEGIN
  ...
  ORDER BY
    (CASE
      WHEN EXISTS (
        SELECT 1 FROM tarefas tp
        WHERE tp.processo_id = p.id
          AND tp.status::text IN ('pendente','atrasado','a_confirmar','em_execucao','revisao','minutado_revisao','reagendado')
      ) THEN 0
      WHEN p.status::text IN ('pendente','urgente') THEN 1
      ELSE 2
    END),
    p.created_at DESC
