DROP FUNCTION IF EXISTS public.get_djen_stats_per_user(uuid, timestamp with time zone, timestamp with time zone);

CREATE OR REPLACE FUNCTION public.get_djen_publicacoes_unificadas(p_coordenacao_id uuid DEFAULT NULL::uuid, p_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_fim timestamp with time zone DEFAULT NULL::timestamp with time zone, p_apenas_nao_lidas boolean DEFAULT false, p_search_query text DEFAULT NULL::text, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_monitoramento_id uuid DEFAULT NULL::uuid, p_tipo_origem text DEFAULT NULL::text, p_read_status text DEFAULT 'todas'::text)
...
BEGIN
  ...
  WITH base AS (
    SELECT pd.id, 'termo'::text AS tipo_origem, NULL::uuid AS processo_id,
      ...
      pd.dedup_processo_digits, pd.dedup_data_ref, pd.dedup_head_norm,
    ...
    SELECT pdp.id, 'processo'::text, pdp.processo_id, pdp.processo_numero, pdp.conteudo,
      ...
      pdp.coordenacao_id, pdp.dedup_processo_digits, pdp.dedup_data_ref, pdp.dedup_head_norm,
...
CREATE OR REPLACE FUNCTION public.get_djen_stats_per_user(p_coordenacao_id uuid DEFAULT NULL, p_inicio timestamp with time zone DEFAULT NULL, p_fim timestamp with time zone DEFAULT NULL)
 RETURNS TABLE(total bigint, lidas bigint, nao_lidas bigint)
...
BEGIN
  ...
  RETURN QUERY
  WITH base AS (
    SELECT pd.id, pd.created_at, COALESCE(pd.coordenacao_id, md.coordenacao_id) AS coord,
           pd.dedup_processo_digits, pd.dedup_data_ref, pd.dedup_head_norm, 'termo'::text AS tabela
    FROM public.publicacoes_djen pd
    JOIN public.monitoramentos_djen md ON md.id = pd.monitoramento_id
    WHERE pd.status IN ('encontrada','duplicada') AND COALESCE(pd.fonte,'') <> 'dejt-pdf'
      AND (p_coordenacao_id IS NULL OR COALESCE(pd.coordenacao_id, md.coordenacao_id) = p_coordenacao_id)
      AND (p_inicio IS NULL OR pd.created_at >= p_inicio)
      AND (p_fim IS NULL OR pd.created_at <= p_fim)
      AND (v_is_admin OR EXISTS (SELECT 1 FROM public.membros_coordenacao mc WHERE mc.coordenacao_id = COALESCE(pd.coordenacao_id, md.coordenacao_id) AND mc.usuario_id = v_uid))
    UNION ALL
    SELECT pdp.id, pdp.created_at, pdp.coordenacao_id, pdp.dedup_processo_digits, pdp.dedup_data_ref, pdp.dedup_head_norm, 'processo'
    FROM public.publicacoes_djen_processos pdp
    WHERE pdp.status IN ('encontrada','duplicada')
  ...
  dedup AS (
    SELECT DISTINCT ON (coord, dedup_processo_digits, dedup_data_ref, dedup_head_norm) id, tabela
    FROM base ORDER BY coord, dedup_processo_digits, dedup_data_ref, dedup_head_norm, created_at DESC, id DESC
  )
  SELECT COUNT(*)::bigint,
    COUNT(*) FILTER (WHERE EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id=d.id AND l.tabela_origem=d.tabela AND l.usuario_id=v_uid))::bigint,
    COUNT(*) FILTER (WHERE NOT EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id=d.id AND l.tabela_origem=d.tabela AND l.usuario_id=v_uid))::bigint
  FROM dedup d;
END;
$function$;