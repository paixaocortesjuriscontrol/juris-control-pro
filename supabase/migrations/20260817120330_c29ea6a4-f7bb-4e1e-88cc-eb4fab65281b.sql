...
tar AS (
 SELECT b.*,CASE WHEN b.origem IN ('astrea','projuris','importacao','import','planilha','migracao','carga','benner') AND a.data_original IS NOT NULL THEN a.data_original ELSE b.data_cumprimento END data_conclusao_filtro
...
abertura AS (
 SELECT t.id,t.tipo,t.criado_por uid FROM tar t WHERE t.criado_por IS NOT NULL AND t.created_at::date BETWEEN p_inicio AND p_fim
 UNION SELECT t.id,t.tipo,r.usuario_id FROM tar t JOIN resp r ON r.tarefa_id=t.id WHERE r.usuario_id IS NOT NULL AND t.criado_por IS NULL AND t.created_at::date BETWEEN p_inicio AND p_fim
),
abertos AS (
