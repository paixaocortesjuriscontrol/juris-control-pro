...
resp AS (
  ...
  UNION
  SELECT te.tarefa_id, te.usuario_id FROM public.tarefa_envolvidos te
  UNION
...
-- Atividades (subatividades) em que o usuário é responsável, envolvido (via item) ou concluiu
ativ_base AS (
  SELECT DISTINCT s.id, u.uid, s.situacao, s.concluida_em, s.data_prevista, s.created_at
  FROM public.subatividades_item s
  LEFT JOIN tar t ON t.id = s.item_id
  CROSS JOIN LATERAL (
    SELECT s.responsavel_id AS uid
    UNION SELECT s.concluida_por
    UNION SELECT r.usuario_id FROM resp r WHERE r.tarefa_id = s.item_id
  ) u
...
ativ AS (
  SELECT uid,
    count(*) FILTER (
      WHERE created_at::date <= p_fim
        AND (coalesce(data_prevista, created_at::date) >= p_inicio OR (concluida_em IS NOT NULL AND concluida_em::date >= p_inicio))
    ) AS at_total,
    count(*) FILTER (
      WHERE concluida_em IS NOT NULL
        AND concluida_em::date >= p_inicio AND concluida_em::date <= p_fim
    ) AS at_concl
  FROM ativ_base
  GROUP BY uid
),
