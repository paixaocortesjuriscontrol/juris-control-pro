-- Restaura a assinatura completa da RPC get_djen_publicacoes_unificadas
-- (foi encolhida em uma migração anterior e o cliente ainda passa 15 params,
-- causando erro "function does not exist" e a lista ficar vazia — inclusive Kurier).
-- Mantém o alinhamento com a RPC de stats: quando p_dedup=true, usa a chave por
-- CONTEÚDO (compute_djen_conteudo_dedup_key) para bater com o "Total Hoje".
-- Kurier: filtro de data usa data_disponibilizacao (com fallback para created_at
-- BRT — "data da captura").

DROP FUNCTION IF EXISTS public.get_djen_publicacoes_unificadas(uuid, timestamptz, timestamptz, boolean, text, integer, integer, uuid, text, text);
    ...
    WHERE (p_coordenacao_id IS NULL OR pd.coordenacao_id = p_coordenacao_id
      ...
      AND (p_inicio IS NULL OR CASE
            WHEN lower(COALESCE(pd.fonte,'')) = 'kurier'
              THEN COALESCE((pd.data_disponibilizacao AT TIME ZONE 'UTC')::date,
                            (pd.created_at AT TIME ZONE 'America/Sao_Paulo')::date)
    ...
    WHERE (p_coordenacao_id IS NULL OR pdp.coordenacao_id = p_coordenacao_id)
      ...
      AND (p_inicio IS NULL OR CASE
            WHEN lower(COALESCE(pdp.fonte,'')) = 'kurier'
              THEN COALESCE((pdp.data_disponibilizacao AT TIME ZONE 'UTC')::date,
                            (pdp.created_at AT TIME ZONE 'America/Sao_Paulo')::date)
                   >= (p_inicio AT TIME ZONE 'America/Sao_Paulo')::date
