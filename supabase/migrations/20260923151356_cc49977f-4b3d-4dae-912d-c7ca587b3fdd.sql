ALTER TABLE public.publicacoes_djen_processos
  ADD COLUMN IF NOT EXISTS monitoramento_id uuid NULL REFERENCES public.monitoramentos_djen(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_pdp_monitoramento_id ON public.publicacoes_djen_processos(monitoramento_id);
...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT pdp.id, 'processo'::text, pdp.processo_id, pdp.processo_numero, pdp.conteudo,
      ...
      pdp.monitoramento_id, md2.termo_busca, md2.descricao, md2.tipo, md2.oab, md2.uf,
    ...
    LEFT JOIN public.monitoramentos_djen md2 ON md2.id = pdp.monitoramento_id
