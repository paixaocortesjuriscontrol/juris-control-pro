...
BEGIN
  ...
  SELECT id FROM (
    ...
    WHERE COALESCE(md.coordenacao_id, pd.coordenacao_id) = p_coordenacao_id
      ...
      AND (p_data_disp_inicio IS NULL OR pd.data_disponibilizacao::date >= p_data_disp_inicio)
      AND (p_data_disp_fim IS NULL OR pd.data_disponibilizacao::date <= p_data_disp_fim)
  ...
  SELECT id FROM (
    ...
    WHERE pdp.coordenacao_id = p_coordenacao_id
      ...
      AND (p_data_disp_inicio IS NULL OR pdp.data_disponibilizacao::date >= p_data_disp_inicio)
      AND (p_data_disp_fim IS NULL OR pdp.data_disponibilizacao::date <= p_data_disp_fim)
      AND public.compute_djen_conteudo_dedup_key(
