...
-- Créditos de abertura: itens criados por robô/importação são creditados aos responsáveis
abertura AS (
  SELECT t.id, t.tipo, t.criado_por AS uid
  FROM tar t
  WHERE t.criado_por IS NOT NULL
    AND (t.origem = '' OR t.origem NOT IN ('astrea','projuris','importacao','analise_djen','publicacao','robo','sistema'))
    AND t.created_at::date >= p_inicio AND t.created_at::date <= p_fim
  UNION
  SELECT tr.id, tr.tipo, tr.uid
  FROM tar_resp tr
  WHERE tr.uid IS NOT NULL
    AND (tr.criado_por IS NULL OR tr.origem IN ('astrea','projuris','importacao','analise_djen','publicacao','robo','sistema'))
