-- Primeiro, dropar as duas versões da função (com e sem _cliente_ids)
DROP FUNCTION IF EXISTS public.get_processos_paginados(integer,integer,text,text,text,uuid,uuid,text,boolean,boolean,timestamp with time zone,timestamp with time zone);
DROP FUNCTION IF EXISTS public.get_processos_paginados(integer,integer,text,text,text,uuid,uuid,text,boolean,boolean,timestamp with time zone,timestamp with time zone,uuid[]);

-- Recriar função otimizada com PL/pgSQL para evitar COUNT(*) OVER() lento
