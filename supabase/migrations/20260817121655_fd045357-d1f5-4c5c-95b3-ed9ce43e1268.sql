...
abertura AS (
 -- Tarefas criadas manualmente no sistema: crédito para quem criou
 ...
 SELECT t.id,t.tipo,r.usuario_id FROM tar t JOIN resp r ON r.tarefa_id=t.id
  WHERE (t.importada OR t.criado_por IS NULL) AND r.usuario_id IS NOT NULL AND t.created_at::date BETWEEN p_inicio AND p_fim
),
abertos AS (
