-- Atualiza a RPC para usar a mesma lógica de deduplicação do frontend (sem monitoramento_id)
  ...
  WITH
    ...
    counts AS (
      SELECT
        ...
        -- DJEN: deduplicação por coordenação + processo_digits + data + conteúdo (300 chars)
        -- NÃO usa monitoramento_id para evitar contagem múltipla de termos duplicados
        (
          WITH djen_union AS (
          ...
          -- CHAVE UNIFICADA: coordenacao + processo_digits + data + head
          -- Não inclui monitoramento_id/tipo_origem para unificar termos duplicados
