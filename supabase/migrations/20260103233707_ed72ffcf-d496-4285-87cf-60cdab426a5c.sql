...
BEGIN
  ...
  -- Processos MPT por STATUS da planilha (via situacao_original). Inclui todas as situações esperadas.
  WITH mpt_base AS (
    SELECT
      COALESCE(NULLIF(btrim(situacao_original), ''), NULLIF(btrim(status::text), ''), 'sem_situacao') AS raw_status
    FROM public.processos
  ...
  mpt_norm AS (
    SELECT
      CASE
        WHEN lower(raw_status) LIKE '%desativad%' THEN 'desativado'
        WHEN lower(raw_status) LIKE '%convertid%' THEN 'convertido'
        WHEN lower(raw_status) LIKE '%arquiv%' THEN 'arquivado'
        WHEN lower(raw_status) LIKE '%sem%' THEN 'sem_situacao'
        -- cuidado: 'desativado' contém 'ativo', por isso o case acima vem antes
        WHEN lower(raw_status) LIKE '%ativo%' THEN 'ativo'
        ELSE 'sem_situacao'
      END AS status_key
    FROM mpt_base
  ),
  mpt_counts AS (
    SELECT status_key, COUNT(*)::int AS total
    FROM mpt_norm
    GROUP BY 1
  ),
  status_list AS (
    SELECT * FROM (
      VALUES
        ('ativo', 1, 'Ativo', '#22C55E'),
        ('sem_situacao', 2, 'Sem Situação', '#CBD5E1'),
        ('arquivado', 3, 'Arquivado', '#94A3B8'),
        ('desativado', 4, 'Desativado', '#6B7280'),
        ('convertido', 5, 'Convertido', '#F59E0B')
    ) AS v(status_key, ord, label, color)
  )
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
  ...
  FROM status_list s
  LEFT JOIN mpt_counts c ON c.status_key = s.status_key;
