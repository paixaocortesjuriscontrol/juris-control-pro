...
BEGIN
  ...
  WITH base AS MATERIALIZED (
    ...
    SELECT
      ...
      NULLIF(
        CASE
          WHEN NULLIF(b.id_djen, '') IS NOT NULL THEN concat_ws('|',
            'id_djen',
            b.coord_id::text,
            NULLIF(b.id_djen, '')
          )
          WHEN b.fonte = 'kurier' THEN concat_ws('|',
            'kurier',
            b.coord_id::text,
            b.processo_digits,
            b.data_ref::text,
      ...
      CASE WHEN NULLIF(b.id_djen, '') IS NOT NULL THEN 0 ELSE 1 END AS prio_id_djen,
      CASE WHEN b.fonte = 'kurier' THEN 0 ELSE 1 END AS prio_fonte_kurier,
    ...
    SELECT
      ...
      ROW_NUMBER() OVER (
        ...
        ORDER BY n.prio_id_djen ASC, n.prio_fonte_kurier ASC, n.prio_id_kurier ASC, n.created_at ASC, n.id ASC
      ) AS rn
    FROM normalizados n
  ...
  RETURN jsonb_build_object(
    ...
    'data_fim', v_data_fim,
    'regra', 'mesma_coordenação: id_djen; fallback kurier/djen por processo + data_ref + conteúdo normalizado; janela por disponibilização, publicação ou captura'
  );
END;
$function$;
