...
BEGIN
  ...
  SELECT id FROM (
    SELECT p.id,
      ROW_NUMBER() OVER (
        PARTITION BY
          ...
          md5(public.djen_normalize_conteudo_sem_destinatarios(p.conteudo))
    ...
    WHERE p.coordenacao_id = p_coordenacao_id
      ...
      AND public.djen_normalize_conteudo_sem_destinatarios(p.conteudo) <> ''
      AND (
  ...
  RETURN jsonb_build_object(
    ...
    'regra', 'coordenacao+processo+conteudo_sem_intimados',
    'origem', 'servidor'
  );
END;
$function$;