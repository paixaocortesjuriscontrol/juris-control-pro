-- Atualizar função get_processos_paginados para incluir filtro de tipo_processo
...
BEGIN
  ...
  WHERE
    ...
    AND (_search IS NULL OR (
      ...
      OR COALESCE(p.auto_infracao,'') ILIKE ('%' || _search || '%')
      OR COALESCE(p.cnpj_fiscalizado,'') ILIKE ('%' || _search || '%')
    ))
