-- Função única para paginação de processos com filtros + existência (DJEN/Movimentações)
-- Observação: SECURITY INVOKER para respeitar RLS do usuário.
...
-- Índices para acelerar EXISTS (caso não existam)
CREATE INDEX IF NOT EXISTS idx_publicacoes_djen_processos_processo_id
  ON public.publicacoes_djen_processos (processo_id);

CREATE INDEX IF NOT EXISTS idx_movimentacoes_processo_id
  ON public.movimentacoes (processo_id);
