CREATE OR REPLACE FUNCTION public.get_ranking_atendimento_geral(p_inicio date, p_fim date, p_coordenacao_id uuid DEFAULT NULL, p_usuario_id uuid DEFAULT NULL)
...
perdidos AS (
 SELECT uid,count(*) p_total FROM tar_resp
 WHERE NOT importada AND prazo IS NOT NULL AND prazo BETWEEN p_inicio AND p_fim
   AND prazo < current_date
   AND status NOT IN ('cumprido','tratado','protocolado','baixado','verificado','cancelado','concluido_sem_sucesso')
 GROUP BY uid
),
ativ_base AS (
