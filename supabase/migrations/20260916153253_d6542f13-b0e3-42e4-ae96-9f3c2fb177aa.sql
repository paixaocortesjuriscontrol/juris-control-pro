...
porresp AS (
  SELECT r.usuario_id AS uid,
         ...
         (b.sem_pendencia IS TRUE) AS marcador_sem_pend
  FROM base b
...
SELECT
  ...
  count(*) AS total,
  -- Mesma regra da tela Distribuição TST: sem/com pendência sempre dentro
  -- do conjunto de processos marcados como Pronto no período.
  count(*) FILTER (WHERE pr.pronto_periodo AND pr.marcador_sem_pend) AS sem_pendencia,
  count(*) FILTER (WHERE pr.pronto_periodo AND NOT pr.marcador_sem_pend) AS com_pendencia,
...
ORDER BY count(*) FILTER (WHERE pr.pronto_periodo) DESC, count(*) FILTER (WHERE pr.pronto_periodo AND pr.marcador_sem_pend) DESC, count(*) DESC;
$function$;