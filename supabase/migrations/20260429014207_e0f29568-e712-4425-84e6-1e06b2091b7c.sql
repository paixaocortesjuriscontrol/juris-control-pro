DROP FUNCTION IF EXISTS public.get_djen_publicacoes_unificadas(uuid, timestamp with time zone, timestamp with time zone, boolean, text, integer, integer, uuid, text, text);
...
RETURNS TABLE(
  ...
  coordenacao_id uuid, coordenacao_nome text, polo_ativo text, polo_passivo text,
  tribunal text, orgao text, tipo_comunicacao text, meio text,
...
BEGIN
  ...
  SELECT f.id, f.tipo_origem, f.processo_id, f.processo_numero, f.conteudo, f.data_publicacao, f.data_disponibilizacao,
    f.fonte, f.lida_por_user AS lida, f.created_at, f.monitoramento_id, f.monitoramento_termo, f.monitoramento_descricao,
