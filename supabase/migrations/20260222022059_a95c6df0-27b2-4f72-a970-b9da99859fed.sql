...
DROP FUNCTION IF EXISTS public.get_djen_publicacoes_unificadas(uuid, timestamp with time zone, timestamp with time zone, boolean, text, integer, integer);

CREATE OR REPLACE FUNCTION public.get_djen_publicacoes_unificadas(p_coordenacao_id uuid, p_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_fim timestamp with time zone DEFAULT NULL::timestamp with time zone, p_apenas_nao_lidas boolean DEFAULT false, p_search_query text DEFAULT NULL::text, p_limit integer DEFAULT 200, p_offset integer DEFAULT 0)
  ...
  WITH
    base AS (
      ...
      SELECT
        ...
        p.tribunal,
        NULL::text AS orgao,
        NULL::text AS tipo_comunicacao,
        NULL::text AS meio,
        NULL::jsonb AS advogados_json,
        NULL::jsonb AS partes_json,
        (
