-- Backfill processo_id em intimações (todas possuem processo_numero com match)
UPDATE public.intimacoes_detectadas i
SET processo_id = p.id
FROM public.processos p
WHERE i.processo_id IS NULL
  AND i.processo_numero IS NOT NULL
  AND p.numero = i.processo_numero;

-- Índice para otimizar filtros/joins
CREATE INDEX IF NOT EXISTS idx_intimacoes_detectadas_processo_id
ON public.intimacoes_detectadas(processo_id);

-- Fix linter: set search_path explícito
