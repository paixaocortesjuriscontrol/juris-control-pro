-- Adicionar filtros de audiências e intimações na RPC get_processos_paginados
...
BEGIN
  ...
  -- Contar total de registros (query simples e rápida com índices)
  SELECT COUNT(*)
  INTO v_total_count
  ...
  WHERE
    ...
    AND (NOT _com_intimacao OR EXISTS (
      SELECT 1 FROM public.intimacoes_detectadas id WHERE id.processo_id = p.id OR id.processo_numero = p.numero
    ))
    AND (_cliente_ids IS NULL OR p.cliente_id = ANY(_cliente_ids));

  -- Retornar registros paginados com total pré-calculado
  RETURN QUERY
  SELECT
    p.id,
    p.numero,
...
-- Criar índices para melhorar performance das consultas de audiências e intimações por processo_numero
CREATE INDEX IF NOT EXISTS idx_audiencias_detectadas_processo_numero ON public.audiencias_detectadas(processo_numero);
CREATE INDEX IF NOT EXISTS idx_intimacoes_detectadas_processo_numero ON public.intimacoes_detectadas(processo_numero);