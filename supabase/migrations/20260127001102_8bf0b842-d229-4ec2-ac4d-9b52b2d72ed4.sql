-- Fix: compute 'total' inside the RPC and remove the unused view.

DROP VIEW IF EXISTS public.vw_notificacoes_counts_by_coordenacao;
  ...
  WITH
    ...
    counts AS (
      SELECT
        ...
        -- Distribuições: mapear por processo_id (quando disponível)
        (
          SELECT COUNT(*)::int
        ...
        -- Andamentos (exclui Redistribuições)
        (
          SELECT COUNT(*)::int
          FROM public.movimentacoes m
          JOIN public.processos p ON p.id = m.processo_id
