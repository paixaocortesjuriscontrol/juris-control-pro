-- Fix: Análise DJEN deve filtrar por data_publicacao (data no DJEN),
-- não por created_at (quando inserimos no banco).
-- Quando o usuário busca execução 03/02, quer ver publicações que saíram no DJEN em 03/02,
-- mesmo que tenham sido inseridas hoje (04/02).
...
BEGIN
  ...
  WITH
    base AS (
      -- TERMO: filtrar por data_publicacao (data no DJEN), fallback para created_at
      SELECT
        pd.id,
      ...
      WHERE md.coordenacao_id = p_coordenacao_id
        AND (p_inicio IS NULL OR COALESCE(pd.data_publicacao, pd.data_disponibilizacao, pd.created_at) >= p_inicio)
        AND (p_fim IS NULL OR COALESCE(pd.data_publicacao, pd.data_disponibilizacao, pd.created_at) <= p_fim)
      ...
      -- PROCESSO: filtrar por data_publicacao (data no DJEN), fallback para created_at
      SELECT
        pdp.id,
      ...
      WHERE p.coordenacao_id = p_coordenacao_id
        AND (p_inicio IS NULL OR COALESCE(pdp.data_publicacao, pdp.data_disponibilizacao, pdp.created_at) >= p_inicio)
        AND (p_fim IS NULL OR COALESCE(pdp.data_publicacao, pdp.data_disponibilizacao, pdp.created_at) <= p_fim)
