-- Drop and recreate the function with all required parameters
DROP FUNCTION IF EXISTS public.get_processos_paginados(
  integer, integer, text, text, text, uuid, uuid, text, 
  boolean, boolean, timestamp with time zone, timestamp with time zone, uuid[]
);
