...
DROP FUNCTION IF EXISTS public.count_djen_publicacoes_unificadas(uuid, timestamptz, timestamptz, boolean, text, uuid);

-- Helper function to strip destinatário section from content for dedup
CREATE OR REPLACE FUNCTION public.strip_destinatarios(content text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN position('Destinatário(s):' IN content) > 0
      THEN left(content, position('Destinatário(s):' IN content) - 1)
    WHEN position('Destinatario(s):' IN content) > 0
      THEN left(content, position('Destinatario(s):' IN content) - 1)
    ELSE content
  END;
$$;

-- Recreate get_djen_publicacoes_unificadas with strip_destinatarios in dedup_key
...
BEGIN
  ...
  WITH
    base AS (
      ...
      JOIN public.coordenacoes c2 ON c2.id = p.coordenacao_id
    ...
    ranked AS (
      SELECT DISTINCT ON (b.dedup_key)
        b.id, b.tipo_origem, b.processo_id, b.processo_numero,
        b.conteudo, b.data_publicacao, b.data_disponibilizacao,
        b.fonte, b.lida, b.created_at,
        b.monitoramento_id, b.monitoramento_termo, b.monitoramento_descricao,
        b.monitoramento_tipo, b.monitoramento_oab, b.monitoramento_uf,
        b.coordenacao_id, b.coordenacao_nome,
        b.polo_ativo, b.polo_passivo, b.tribunal,
        b.orgao, b.tipo_comunicacao, b.meio,
        b.advogados_json, b.partes_json
      FROM base b
