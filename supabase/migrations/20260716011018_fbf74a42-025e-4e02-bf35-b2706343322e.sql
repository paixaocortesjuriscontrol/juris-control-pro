...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE (p_coordenacao_id IS NULL OR pd.coordenacao_id = p_coordenacao_id OR (pd.coordenacao_id IS NULL AND md.coordenacao_id = p_coordenacao_id))
      ...
      -- Quando o filtro Tipo de Origem NÃO está definido (aba "Todas"),
      -- incluímos também as Pautas DEJT (fonte 'dejt-pdf'). Em filtros
      -- específicos (termo/parte/kurier), as pautas continuam ocultas para
      -- não misturar com intimações.
