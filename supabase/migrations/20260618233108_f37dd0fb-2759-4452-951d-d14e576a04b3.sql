...
SET search_path TO 'public'
AS $$
  SELECT COALESCE(to_char(d.data_distribuicao_real, 'YYYY-MM'), 'sem-data') AS mes_ano,
         count(*) AS total
  FROM public.dados_benner d
  WHERE d.aba_origem IS NOT NULL
  GROUP BY 1
  ORDER BY CASE WHEN COALESCE(to_char(d.data_distribuicao_real, 'YYYY-MM'), 'sem-data') = 'sem-data' THEN 1 ELSE 0 END,
           COALESCE(to_char(d.data_distribuicao_real, 'YYYY-MM'), 'sem-data') DESC;
$$;
