...
BEGIN
  ...
  IF NOT COALESCE(p_dedup, false) THEN
    RETURN QUERY
    WITH base AS (
      SELECT pd.id, 'termo'::text AS tipo_origem, NULL::uuid AS processo_id,
        ...
        pd.fonte, pd.created_at, pd.monitoramento_id,
      ...
      WHERE (
        ...
        AND (
          ...
          OR CASE
            WHEN lower(COALESCE(pd.fonte, '')) = 'kurier'
              THEN COALESCE((pd.data_disponibilizacao AT TIME ZONE 'UTC')::date, (pd.created_at AT TIME ZONE 'America/Sao_Paulo')::date) >= (p_inicio AT TIME ZONE 'America/Sao_Paulo')::date
        ...
        AND (
          ...
          OR CASE
            WHEN lower(COALESCE(pd.fonte, '')) = 'kurier'
              THEN COALESCE((pd.data_disponibilizacao AT TIME ZONE 'UTC')::date, (pd.created_at AT TIME ZONE 'America/Sao_Paulo')::date) <= (p_fim AT TIME ZONE 'America/Sao_Paulo')::date
        ...
        AND (NOT p_apenas_nao_lidas OR NOT EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = pd.id AND l.tabela_origem = 'termo' AND l.usuario_id = v_uid))
        AND (
          v_read = 'todas'
          OR (v_read = 'nao_lidas' AND NOT EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = pd.id AND l.tabela_origem = 'termo' AND l.usuario_id = v_uid))
          OR (v_read = 'lidas' AND EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = pd.id AND l.tabela_origem = 'termo' AND l.usuario_id = v_uid))
        )
      ...
      SELECT pdp.id, 'processo'::text, pdp.processo_id, pdp.processo_numero, pdp.conteudo,
        pdp.data_publicacao, pdp.data_disponibilizacao, pdp.fonte, pdp.created_at,
      ...
      WHERE (p_coordenacao_id IS NULL OR pdp.coordenacao_id = p_coordenacao_id)
        ...
        AND (
          ...
          OR CASE
            WHEN lower(COALESCE(pdp.fonte, '')) = 'kurier'
              THEN COALESCE((pdp.data_disponibilizacao AT TIME ZONE 'UTC')::date, (pdp.created_at AT TIME ZONE 'America/Sao_Paulo')::date) >= (p_inicio AT TIME ZONE 'America/Sao_Paulo')::date
        ...
        AND (
          ...
          OR CASE
            WHEN lower(COALESCE(pdp.fonte, '')) = 'kurier'
              THEN COALESCE((pdp.data_disponibilizacao AT TIME ZONE 'UTC')::date, (pdp.created_at AT TIME ZONE 'America/Sao_Paulo')::date) <= (p_fim AT TIME ZONE 'America/Sao_Paulo')::date
        ...
        AND (NOT p_apenas_nao_lidas OR NOT EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = pdp.id AND l.tabela_origem = 'processo' AND l.usuario_id = v_uid))
        AND (
          v_read = 'todas'
          OR (v_read = 'nao_lidas' AND NOT EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = pdp.id AND l.tabela_origem = 'processo' AND l.usuario_id = v_uid))
          OR (v_read = 'lidas' AND EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = pdp.id AND l.tabela_origem = 'processo' AND l.usuario_id = v_uid))
        )
    ...
    final_rows AS (
      SELECT b.*,
        EXISTS (SELECT 1 FROM public.publicacoes_djen_leituras l WHERE l.publicacao_id = b.id AND l.tabela_origem = b.tipo_origem AND l.usuario_id = v_uid) AS lida_por_user
      FROM base b
...
CREATE INDEX IF NOT EXISTS idx_djen_leituras_lookup_usuario_pub
  ON public.publicacoes_djen_leituras (usuario_id, tabela_origem, publicacao_id);

CREATE INDEX IF NOT EXISTS idx_publicacoes_djen_status_created
  ON public.publicacoes_djen (status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_publicacoes_djen_coord_created
  ON public.publicacoes_djen (coordenacao_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_publicacoes_djen_processos_status_created
  ON public.publicacoes_djen_processos (status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_publicacoes_djen_processos_coord_created
