...
BEGIN
  ...
  -- Excluir itens tratados (data_cumprimento / prazo_fatal_conferido / tratado_em / concluido_em)
  -- quando não estamos filtrando explicitamente por 'cumprido'/'concluido'/'tratado'.
  ...
  WITH
    ...
    djen_raw AS (
      SELECT
        ...
        COALESCE(to_char(pd.data_publicacao::date, 'YYYY-MM-DD'),
                 to_char(pd.data_disponibilizacao::date, 'YYYY-MM-DD'),
                 to_char(pd.created_at::date, 'YYYY-MM-DD')) AS data_ref,
      ...
      SELECT
        ...
        COALESCE(to_char(pdp.data_publicacao::date, 'YYYY-MM-DD'),
                 to_char(pdp.data_disponibilizacao::date, 'YYYY-MM-DD'),
                 to_char(pdp.created_at::date, 'YYYY-MM-DD')) AS data_ref,
        left(lower(regexp_replace(regexp_replace(regexp_replace(
    ...
    djen_dedup AS (
      SELECT cid, COUNT(DISTINCT cid::text || '|' || processo_digits || '|' || data_ref || '|' || head_norm)::int AS cnt
      FROM djen_raw GROUP BY cid
    ),
    dist AS (
      ...
      WHERE p.coordenacao_id = ANY(SELECT cid FROM allowed)
        AND (v_status_filter IS NULL OR de.status = (CASE WHEN v_status_filter = 'concluido' THEN 'tratado' ELSE v_status_filter END))
    ...
    redist AS (
      ...
      JOIN processos p ON p.id = am.processo_id
      WHERE p.coordenacao_id = ANY(SELECT cid FROM allowed)
        AND am.tipo_alerta = 'redistribuicao'
        AND (v_status_filter IS NULL OR am.status = (CASE WHEN v_status_filter = 'concluido' THEN 'tratado' ELSE v_status_filter END))
        AND (v_d_ini IS NULL OR am.created_at >= v_d_ini)
        AND (v_d_fim IS NULL OR am.created_at <= v_d_fim)
      GROUP BY p.coordenacao_id
    ...
    prazos_cnt AS (
      ...
      WHERE p.coordenacao_id = ANY(SELECT cid FROM allowed)
        ...
        AND t.data_cumprimento IS NULL
