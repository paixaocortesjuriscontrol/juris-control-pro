...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO public SET statement_timeout = '90s' AS $function$
WITH todos AS (
  ...
  FROM dados_benner b
  WHERE (p_coordenacao_id IS NULL
         OR p_coordenacao_id = 'b0f690ad-68da-43d7-af5f-9adafeab3fd5'::uuid
         OR b.coordenacao_id = p_coordenacao_id)
    AND (p_equipe IS NULL OR trim(b.equipe) = p_equipe)
    AND (p_tribunal IS NULL OR b.tribunal = p_tribunal)
),
ger AS (SELECT GREATEST(avg(ac), 0.001) p FROM todos),
...
fatores AS (
  SELECT a.id,
    ...
    g.p
  FROM abertos a CROSS JOIN ger g
...
SELECT jsonb_build_object(
  'taxa_geral', (SELECT round(p*1000)/10 FROM ger),
