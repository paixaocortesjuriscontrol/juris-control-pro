  ...
  WITH base AS (
    ...
    WHERE (p_coordenacao_id IS NULL OR COALESCE(pd.coordenacao_id, md.coordenacao_id) = p_coordenacao_id)
      ...
      AND (
        ...
        OR CASE
          WHEN lower(COALESCE(pd.fonte, '')) = 'kurier'
            THEN COALESCE(pd.data_publicacao, pd.created_at)::date >= (p_inicio AT TIME ZONE 'America/Sao_Paulo')::date
      ...
      AND (
        ...
        OR CASE
          WHEN lower(COALESCE(pd.fonte, '')) = 'kurier'
            THEN COALESCE(pd.data_publicacao, pd.created_at)::date <= (p_fim AT TIME ZONE 'America/Sao_Paulo')::date
    ...
    WHERE (p_coordenacao_id IS NULL OR pdp.coordenacao_id = p_coordenacao_id)
      ...
      AND (
        ...
        OR CASE
          WHEN lower(COALESCE(pdp.fonte, '')) = 'kurier'
            THEN COALESCE(pdp.data_publicacao, pdp.created_at)::date >= (p_inicio AT TIME ZONE 'America/Sao_Paulo')::date
      ...
      AND (
        ...
        OR CASE
          WHEN lower(COALESCE(pdp.fonte, '')) = 'kurier'
            THEN COALESCE(pdp.data_publicacao, pdp.created_at)::date <= (p_fim AT TIME ZONE 'America/Sao_Paulo')::date
