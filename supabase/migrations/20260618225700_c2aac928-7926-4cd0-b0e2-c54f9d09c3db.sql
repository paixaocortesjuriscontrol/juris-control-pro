...
BEGIN
  ...
  -- Limpa marcador ic_duplicado em registros que ficaram sozinhos (sem pares ativos)
  WITH afetados AS (
    SELECT DISTINCT pn FROM _losers
  ),
  ainda_ativos AS (
    SELECT regexp_replace(coalesce(d.processo,''), '\D', '', 'g') AS pn, d.id
    FROM public.dados_benner d
    JOIN afetados a ON a.pn = regexp_replace(coalesce(d.processo,''), '\D', '', 'g')
  ),
  sem_par AS (
    SELECT pn FROM ainda_ativos GROUP BY pn HAVING count(*) = 1
  )
  UPDATE public.dados_benner d
  SET ic_duplicado = false
  FROM ainda_ativos a
  JOIN sem_par s ON s.pn = a.pn
  WHERE d.id = a.id AND coalesce(d.ic_duplicado, false) = true;
