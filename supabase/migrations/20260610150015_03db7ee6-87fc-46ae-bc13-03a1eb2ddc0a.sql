-- 1. Backfill seguro: só formata onde a versão mascarada ainda não existe
UPDATE public.processos p
SET numero = format('%s-%s.%s.%s.%s.%s',
  substring(p.numero,1,7), substring(p.numero,8,2), substring(p.numero,10,4),
  substring(p.numero,14,1), substring(p.numero,15,2), substring(p.numero,17,4))
WHERE p.numero ~ '^[0-9]{20}$'
  AND NOT EXISTS (
    SELECT 1 FROM public.processos p2
    WHERE p2.id <> p.id
      AND p2.numero = format('%s-%s.%s.%s.%s.%s',
        substring(p.numero,1,7), substring(p.numero,8,2), substring(p.numero,10,4),
        substring(p.numero,14,1), substring(p.numero,15,2), substring(p.numero,17,4))
  );

-- 2. Atualiza RPC de busca
