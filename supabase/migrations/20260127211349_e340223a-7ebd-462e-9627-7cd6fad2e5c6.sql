...
-- Atualizar a RPC para contar DJEN com deduplicação usando COUNT(DISTINCT hash_key)
-- A chave de deduplicação é: coordenacao_id + processo_numero (dígitos) + data + primeiros 300 chars do conteúdo normalizado
  ...
  WITH
    ...
    counts AS (
      SELECT
        ...
        -- DJEN: contagem com deduplicação por chave composta (processo + data + conteúdo)
        (
          SELECT COUNT(DISTINCT 
            md5(
              md.coordenacao_id::text || '|' ||
              regexp_replace(COALESCE(pd.processo_numero, ''), '\D', '', 'g') || '|' ||
              COALESCE(pd.data_publicacao::date, pd.created_at::date)::text || '|' ||
              left(regexp_replace(COALESCE(pd.conteudo, ''), '\s+', ' ', 'g'), 300)
            )
          )::int
        ...
        -- Prazos: REGRA FIXA (hoje..hoje+5) usando data_vencimento, data_fatal ou data_base
        (
          SELECT COUNT(*)::int
          FROM public.tarefas t
          JOIN public.processos p ON p.id = t.processo_id
