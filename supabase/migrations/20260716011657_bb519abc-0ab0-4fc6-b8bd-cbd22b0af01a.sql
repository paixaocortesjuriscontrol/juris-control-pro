CREATE OR REPLACE FUNCTION public.get_djen_publicacoes_unificadas(p_coordenacao_id uuid DEFAULT NULL::uuid, p_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_fim timestamp with time zone DEFAULT NULL::timestamp with time zone, p_apenas_nao_lidas boolean DEFAULT false, p_search_query text DEFAULT NULL::text, p_limit integer DEFAULT 500, p_offset integer DEFAULT 0, p_monitoramento_id uuid DEFAULT NULL::uuid, p_tipo_origem text DEFAULT NULL::text, p_read_status text DEFAULT 'todas'::text, p_data_disponibilizacao_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_data_disponibilizacao_fim timestamp with time zone DEFAULT NULL::timestamp with time zone, p_tribunal text DEFAULT NULL::text, p_dedup boolean DEFAULT false)
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE (p_coordenacao_id IS NULL OR pd.coordenacao_id = p_coordenacao_id OR (pd.coordenacao_id IS NULL AND md.coordenacao_id = p_coordenacao_id))
      ...
      -- Aba "Todas" (v_tipo IS NULL): inclui também as Pautas DEJT (fonte 'dejt-pdf').
      -- Filtros específicos: ocultam pautas para não misturar com intimações.
      ...
      -- Para Pautas DEJT, o "dia" legal é a data de publicação (não a data de
      -- disponibilização do caderno). Isso alinha o filtro com o card
      -- "Pautas DEJT" e evita cair fora do intervalo quando o caderno é
      -- disponibilizado em um dia e a pauta é publicada legalmente no dia seguinte.
