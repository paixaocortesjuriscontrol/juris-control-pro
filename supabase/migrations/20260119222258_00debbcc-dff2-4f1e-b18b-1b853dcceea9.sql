-- Recriar a função corrigindo o alias da tabela publicacoes_djen
...
BEGIN
  ...
  -- Calcular total primeiro (query separada para performance)
  ...
  WHERE
    ...
    AND (NOT _com_publicacao_djen OR EXISTS (
      SELECT 1 FROM publicacoes_djen pub 
      WHERE pub.processo_id = p.id
        AND (_periodo_inicio IS NULL OR pub.data_disponibilizacao >= _periodo_inicio::date)
        AND (_periodo_fim IS NULL OR pub.data_disponibilizacao <= _periodo_fim::date)
    ))
