...
tar_resp AS (
 SELECT r.usuario_id uid,t.*,
  (t.prazo IS NOT NULL AND NOT (t.importada AND t.data_fatal IS NULL AND t.data_prevista IS NULL
    AND t.data_conclusao_filtro IS NOT NULL AND t.data_conclusao_filtro::date = t.prazo)) AS avaliavel
 FROM tar t JOIN resp r ON r.tarefa_id=t.id
),
abertura AS (
...
concl AS (
 SELECT uid,count(*) c_total,
  count(*) FILTER(WHERE avaliavel) c_aval,
  count(*) FILTER(WHERE avaliavel AND data_conclusao_filtro::date<=prazo) c_prazo,
  count(*) FILTER(WHERE avaliavel AND data_conclusao_filtro::date>prazo) c_atraso
 FROM tar_resp
...
perdidos AS (
 SELECT uid,count(*) p_total FROM tar_resp WHERE NOT importada AND prazo IS NOT NULL AND prazo BETWEEN p_inicio AND p_fim AND ((status IN ('cumprido','tratado','protocolado','baixado','verificado') AND data_conclusao_filtro IS NOT NULL AND data_conclusao_filtro::date>prazo) OR (status NOT IN ('cumprido','tratado','protocolado','baixado','verificado','cancelado','concluido_sem_sucesso') AND prazo<current_date)) GROUP BY uid
),
ativ_base AS (
