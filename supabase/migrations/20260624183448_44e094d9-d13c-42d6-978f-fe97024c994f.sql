...
-- 1) Índice parcial para o ramo Kurier — usa COALESCE(data_publicacao, created_at) como ordem.
CREATE INDEX IF NOT EXISTS idx_pub_djen_kurier_coord_datapub
  ON public.publicacoes_djen (coordenacao_id, (COALESCE(data_publicacao, created_at)) DESC)
  WHERE lower(COALESCE(fonte, '')) = 'kurier'
    AND status IN ('encontrada','duplicada');

CREATE INDEX IF NOT EXISTS idx_pub_djen_proc_kurier_coord_datapub
  ON public.publicacoes_djen_processos (coordenacao_id, (COALESCE(data_publicacao, created_at)) DESC)
  WHERE lower(COALESCE(fonte, '')) = 'kurier'
    AND status IN ('encontrada','duplicada');

-- 2) RPC sem CASE/::date: filtro virou OR sargável que permite ao planner
--    escolher BitmapOr entre o índice "normal" (created_at) e o índice
--    parcial Kurier acima.
