...
CREATE OR REPLACE FUNCTION public.get_djen_publicacoes_unificadas(p_coordenacao_id uuid, p_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_fim timestamp with time zone DEFAULT NULL::timestamp with time zone, p_apenas_nao_lidas boolean DEFAULT false, p_search_query text DEFAULT NULL::text, p_limit integer DEFAULT 200, p_offset integer DEFAULT 0, p_monitoramento_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  WITH base AS (
    SELECT pd.id, 'termo'::text AS tipo_origem, NULL::uuid AS processo_id, pd.processo_numero, pd.conteudo,
      ...
      NULL::text AS polo_ativo, NULL::text AS polo_passivo, pd.tribunal, pd.orgao, pd.tipo_comunicacao, pd.meio,
  ...
  ranked AS (
    SELECT DISTINCT ON (b.dedup_key)
      b.id, b.tipo_origem, b.processo_id, b.processo_numero, b.conteudo, b.data_publicacao, b.data_disponibilizacao,
      b.fonte, b.lida, b.created_at, b.monitoramento_id, b.monitoramento_termo, b.monitoramento_descricao,
      b.monitoramento_tipo, b.monitoramento_oab, b.monitoramento_uf, b.coordenacao_id, b.coordenacao_nome,
      b.polo_ativo, b.polo_passivo, b.tribunal, b.orgao, b.tipo_comunicacao, b.meio, b.advogados_json, b.partes_json
    FROM base b
    ORDER BY b.dedup_key, b.prio, b.created_at DESC
  )
  SELECT * FROM ranked ORDER BY ranked.created_at DESC LIMIT p_limit OFFSET p_offset;
END;
$function$;
