...
BEGIN
  ...
  WITH base AS (
    SELECT pd.id, 'termo'::text AS tipo_origem, NULL::uuid AS processo_id,
      ...
      COALESCE(NULLIF(btrim(pd.id_djen), ''), concat_ws('|', 'legacy', pd.dedup_processo_digits, pd.dedup_data_ref::text, pd.dedup_head_norm)) AS dedup_uid,
    ...
    SELECT pdp.id, 'processo'::text, pdp.processo_id, pdp.processo_numero, pdp.conteudo,
      ...
      pdp.coordenacao_id,
      COALESCE(NULLIF(btrim(pdp.id_djen), ''), concat_ws('|', 'legacy', pdp.dedup_processo_digits, pdp.dedup_data_ref::text, pdp.dedup_head_norm)),
...
CREATE OR REPLACE FUNCTION public.get_djen_stats_per_user(p_coordenacao_id uuid DEFAULT NULL::uuid, p_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_fim timestamp with time zone DEFAULT NULL::timestamp with time zone, p_tipo_origem text DEFAULT NULL::text, p_search_query text DEFAULT NULL::text, p_monitoramento_id uuid DEFAULT NULL::uuid)
...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT
      ...
      COALESCE(NULLIF(btrim(pdp.id_djen), ''), concat_ws('|', 'legacy', pdp.dedup_processo_digits, pdp.dedup_data_ref::text, pdp.dedup_head_norm)) AS dedup_uid,
      2 AS prio,
...
AS $function$
  WITH seed_keys AS (
    ...
    SELECT DISTINCT
      COALESCE(NULLIF(btrim(pdd.id_djen), ''), concat_ws('|', 'legacy', pdd.dedup_processo_digits, pdd.dedup_data_ref::text, pdd.dedup_head_norm)) AS dedup_uid,
  ...
  JOIN seed_keys sk
   ...
   AND COALESCE(NULLIF(btrim(t.id_djen), ''), concat_ws('|', 'legacy', t.dedup_processo_digits, t.dedup_data_ref::text, t.dedup_head_norm)) = sk.dedup_uid
