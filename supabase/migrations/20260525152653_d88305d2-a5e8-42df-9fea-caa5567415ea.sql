DROP FUNCTION IF EXISTS public.get_distribuicao_tst_responsaveis_counts(jsonb);
...
BEGIN
  ...
  RETURN QUERY
  WITH base AS (
    SELECT db.id, db.status_distribuicao
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE b.status_distribuicao = 'finalizada')::bigint AS pronto
  FROM base b
...
GRANT EXECUTE ON FUNCTION public.get_distribuicao_tst_responsaveis_counts(jsonb) TO authenticated;