...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN p_dedup THEN unicas.total_unicas ELSE bruto.total_bruto END AS total_unicas,
    CASE WHEN p_dedup THEN unicas.nao_lidas_unicas ELSE bruto.nao_lidas_termos END AS nao_lidas_unicas,
    bruto.total_bruto
  FROM bruto CROSS JOIN unicas;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.get_djen_stats_servidor_per_user(uuid, timestamp with time zone, timestamp with time zone, text, text, uuid, timestamp with time zone, timestamp with time zone, text, boolean, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_djen_stats_servidor_per_user(uuid, timestamp with time zone, timestamp with time zone, text, text, uuid, timestamp with time zone, timestamp with time zone, text, boolean, boolean) TO service_role;

CREATE INDEX IF NOT EXISTS idx_pds_coord_tipo_created_desc
  ON public.publicacoes_djen_servidor (coordenacao_id, tipo_publicacao, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_pds_coord_data_pub_desc
  ON public.publicacoes_djen_servidor (coordenacao_id, data_publicacao DESC);

CREATE INDEX IF NOT EXISTS idx_pds_coord_data_disp_desc
  ON public.publicacoes_djen_servidor (coordenacao_id, data_disponibilizacao DESC);

ANALYZE public.publicacoes_djen_servidor;
ANALYZE public.publicacoes_djen_leituras;