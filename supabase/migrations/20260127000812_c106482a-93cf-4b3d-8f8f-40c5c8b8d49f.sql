-- Consolidate notification counters into a single RPC to avoid dozens of parallel client-side requests
-- This greatly reduces 'Failed to fetch' issues and improves dashboard performance.
...
BEGIN
  ...
  SELECT
    ...
    -- Distribuições: tentar mapear por processo_id (quando disponível)
    (
      SELECT COUNT(*)::int
    ...
    -- Redistribuições (movimentações tipo = Redistribuição)
    (
      SELECT COUNT(*)::int
    ...
    -- Andamentos (movimentações tipo != Redistribuição)
    (
      SELECT COUNT(*)::int
      FROM public.movimentacoes m
      JOIN public.processos p ON p.id = m.processo_id
    ...
    -- Tarefas (por coordenação via processo_id)
    (
      SELECT COUNT(*)::int
      ...
      WHERE p.coordenacao_id = a.coordenacao_id
        AND (
          dr.status_filter IS NULL
          OR t.status = (CASE WHEN dr.status_filter = 'concluido' THEN 'cumprido' ELSE dr.status_filter END)
        )
        AND (dr.prioridade_filter IS NULL OR t.prioridade = dr.prioridade_filter)
        AND (
    ...
    -- Prazos (tarefas pendentes com vencimento <= hoje+3)
    (
      SELECT COUNT(*)::int
      FROM public.tarefas t
      JOIN public.processos p ON p.id = t.processo_id
...
-- Fill the 'total' column via a view-like wrapper to keep the RPC output stable
CREATE OR REPLACE VIEW public.vw_notificacoes_counts_by_coordenacao
...
SELECT
  ...
  audiencias,
  intimacoes,
  (djen + distribuicoes + alertas360 + redistribuicoes + andamentos + prazos + tarefas + audiencias + intimacoes)::int AS total
FROM public.get_notificacoes_counts_by_coordenacao(ARRAY[]::uuid[], NULL, NULL, NULL, NULL, NULL);

-- Note: The view is a convenience for SQL inspection only; the app should call the function directly.