...
BEGIN
  ...
  DROP TABLE IF EXISTS _dup_ids;
  DROP TABLE IF EXISTS _dup_base;

  CREATE TEMP TABLE _dup_ids (id uuid PRIMARY KEY) ON COMMIT DROP;

  CREATE TEMP TABLE _dup_base (
    id uuid PRIMARY KEY,
  ...
  WITH base AS MATERIALIZED (
    SELECT pd.id, pd.created_at, COALESCE(pd.coordenacao_id, p_coordenacao_id) AS coord_id,
      pd.fonte, NULLIF(pd.id_djen, '') AS id_djen, NULLIF(pd.id_kurier::text, '') AS id_kurier,
  ...
  -- Passo 2: duplicadas por mesmo id_djen.
  INSERT INTO _dup_ids (id)
  WITH n AS (
