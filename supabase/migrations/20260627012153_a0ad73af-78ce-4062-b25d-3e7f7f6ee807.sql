CREATE OR REPLACE FUNCTION public.djen_normalize_conteudo_descarte_sem_intimados(p_text text)
RETURNS text
LANGUAGE sql
IMMUTABLE
      ...
      regexp_replace(
        regexp_replace(
          regexp_replace(
            regexp_replace(
              COALESCE(p_text, ''),
              '(?is)<br\s*/?>\s*Intimado\(s\)\s*/\s*Citado\(s\).*$',
              '',
              'g'
            ),
            '(?is)(^|[[:space:]])Intimado\(s\)\s*/\s*Citado\(s\).*$',
            '',
            'g'
          ),
          '<[^>]*>',
...
GRANT EXECUTE ON FUNCTION public.djen_normalize_conteudo_descarte_sem_intimados(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.djen_normalize_conteudo_descarte_sem_intimados(text) TO service_role;
...
BEGIN
  ...
  -- Regra exclusiva do botão: coordenação + processo + conteúdo COMPLETO normalizado,
  -- removendo apenas o bloco final de Intimado(s)/Citado(s). Não usa a chave visual
  -- compute_djen_conteudo_dedup_key porque ela é resumida a 400 caracteres.
  ...
  RETURN jsonb_build_object(
    ...
    'regra', 'coordenacao+processo+conteudo_completo_sem_intimados',
    'origem', 'servidor'
  );
END;
$function$;
...
GRANT EXECUTE ON FUNCTION public.descartar_duplicadas_coordenacao_servidor(uuid, date, date) TO service_role;