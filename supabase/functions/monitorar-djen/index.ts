...
// Jina/Browserless API keys for proxy fallback
const JINA_READER_URL = "https://r.jina.ai";
const JINA_API_KEY = Deno.env.get('JINA_API_KEY') || '';
const BROWSERLESS_API_URL = "https://chrome.browserless.io";
const BROWSERLESS_API_KEY = Deno.env.get('BROWSERLESS_API_KEY') || '';

// Rate limiting state for Jina
let lastJinaRequestTime = 0;

// Note: generateTaskDedupKey, verificarTarefaExistente, and criarTarefasParaResponsaveis
// are now imported from ./tarefas.ts

// Note: generateGlobalHash, normalizar, normalizarParaBusca, escapeRegex, extrairPalavraChavePura,
// contemFraseExata, validarTermoComAnd, conteudoContemTermo,
...
import {
  TribunalStats,
  SearchParams,
  processPublicationFromIndex,
  buscarNoIndiceDiario,
  buscarNoIndiceOab,
  processMonitoramentoIndexed,
} from "./processing.ts";
...
async function processMonitoramento(
  ...
  // Use modularized function for indexed mode
  if (options.indexed && options.diarioYmd) {
    return await processMonitoramentoIndexed(supabase, monitoramento, options.diarioYmd);
  }
