...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (novasAudiencias.length > 0) {
      for (const audiencia of novasAudiencias) {
        ...
        if (prioridadeTermo === 'urgente') {
          console.log(`Urgent hearing detected ${inserted.id}; task creation skipped by policy`);
        }
      }
    }
    ...
    // Audiências detectadas por DJEN não viram tarefas automaticamente.
    // O fluxo canônico para audiência humana é o botão Adicionar > Audiência.
