...
serve(async (req) => {
  ...
  try {
    ...
    const requestBody = req.method === "POST"
      ? await req.json().catch(() => ({})) as { acao?: string }
      : {};

    if (requestBody.acao === "limpar_fila_zapi") {
      ...
      const { data: role } = await adminClient
        ...
        .maybeSingle();
      if (!role) {
        return new Response(JSON.stringify({ ok: false, error: "Ação restrita a administradores" }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }

      const queueUrl = `https://api.z-api.io/instances/${ZAPI_INSTANCE_ID}/token/${ZAPI_TOKEN}/queue`;
      ...
      if (!queueResponse.ok) {
        console.error(`[processar-lembretes-audiencia] Falha ao limpar fila Z-API [${queueResponse.status}]: ${queueBody}`);
        return new Response(JSON.stringify({ ok: false, error: "Falha ao limpar fila da Z-API", status: queueResponse.status, details: queueBody }), {
          status: queueResponse.status,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }

      console.log(`[processar-lembretes-audiencia] Fila externa da Z-API limpa pelo admin ${userId}`);
      return new Response(JSON.stringify({ ok: true, fila_zapi_limpa: true }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
