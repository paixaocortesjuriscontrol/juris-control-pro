...
export function PartesProcessoTab({ dadosBennerId, processoNumero }: Props) {
  const { data: partes = [], isLoading } = useQuery<Parte[]>({
    queryKey: ["partes-processo-benner", dadosBennerId, processoNumero],
    enabled: !!dadosBennerId,
    queryFn: async () => {
      if (!dadosBennerId) return [];
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="text-center py-8 text-muted-foreground text-sm">
            ...
            Nenhuma parte registrada. Clique no botão <strong>Judit</strong> no topo do detalhe para importar.
          </div>
        ) : (
