...
export const RelatorioPrintView = forwardRef<HTMLDivElement, RelatorioPrintViewProps>(
  ({ resumoData, atividadesData, clientesData, completoData, mode = "completo" }, ref) => {
    ...
    return (
      <div ref={ref} className="bg-white text-black p-4 pdf-capture-hidden" style={{ fontFamily: 'Arial, sans-serif', maxWidth: '210mm', fontSize: '9px' }}>
        {/* Cabeçalho Executivo */}
        <header className="text-center border-b-2 border-gray-800 pb-2 mb-3">
          <h1 className="text-xl font-bold text-gray-900 mb-1 tracking-wide">RELATÓRIO GERENCIAL</h1>
          <p className="text-sm text-gray-700 font-medium">{subtitle}</p>
          <p className="text-[10px] text-gray-500 mt-1">Gerado em: {dataGeracao}</p>
        </header>
        ...
        {showCompleto && (
          <section className="mb-3">
            <h2 className="text-sm font-bold text-gray-900 border-b border-indigo-600 pb-1 mb-2">
              1. VISÃO GERAL DO ESCRITÓRIO
            </h2>

            <div className="grid grid-cols-7 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-blue-600">{completoData?.dashboardStats?.totalProcessos ?? 0}</p>
                <p className="text-[8px] text-gray-600">Total Processos</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-green-600">{completoData?.dashboardStats?.processosAtivos ?? 0}</p>
                <p className="text-[8px] text-gray-600">Ativos</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-purple-600">{completoData?.dashboardStats?.processosDistribuidos ?? 0}</p>
                <p className="text-[8px] text-gray-600">Distribuídos</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-amber-600">{completoData?.dashboardStats?.processosNaoDistribuidos ?? 0}</p>
                <p className="text-[8px] text-gray-600">Não Distrib.</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-red-600">{completoData?.dashboardStats?.prazosUrgentes ?? 0}</p>
                <p className="text-[8px] text-gray-600">Prazos Urg.</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-blue-600">{completoData?.dashboardStats?.totalAdvogados ?? 0}</p>
                <p className="text-[8px] text-gray-600">Advogados</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-indigo-600">{completoData?.dashboardStats?.totalCoordenacoes ?? 0}</p>
        ...
        {showResumo && resumoData && (
          <section className="mb-3">
            <h2 className="text-sm font-bold text-gray-900 border-b border-blue-600 pb-1 mb-2">
              2. RESUMO EXECUTIVO
            </h2>

            {/* Cards de Resumo - linha única */}
            <div className="grid grid-cols-4 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-blue-600">{resumoData.totalProcessos}</p>
                <p className="text-[8px] text-gray-600">Total Processos</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-green-600">{resumoData.processosAtivosAnoAtual}</p>
                <p className="text-[8px] text-gray-600">Ativos {new Date().getFullYear()}</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-purple-600">{resumoData.mediaEnvolvidos}</p>
                <p className="text-[8px] text-gray-600">Média Envolvidos</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-amber-600">{resumoData.totalMovimentacoes}</p>
                <p className="text-[8px] text-gray-600">Total Andamentos</p>
              </div>
            </div>
            ...
            <div className="mb-2">
              <h3 className="text-[10px] font-semibold text-gray-800 mb-1">2.5 Detalhamento por Área</h3>
        ...
        {showAtividades && atividadesData && (
          <section className="mb-3 page-break-before">
            <h2 className="text-sm font-bold text-gray-900 border-b border-green-600 pb-1 mb-2">
              3. CONTROLE DE ATIVIDADES E PRAZOS
            </h2>

            <div className="grid grid-cols-3 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-gray-800">{atividadesData.totalPrazos}</p>
                <p className="text-[8px] text-gray-600">Total Prazos</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-green-600">{atividadesData.atividadesConcluidas}</p>
                <p className="text-[8px] text-gray-600">Concluídas</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-amber-600">{atividadesData.atividadesNaoConcluidas}</p>
                <p className="text-[8px] text-gray-600">Pend./Atrasadas</p>
              </div>
            </div>
            ...
            {atividadesData.atividadesPorArea?.some((a: any) => a.concluidas > 0 || a.pendentes > 0) && (
              <div className="mb-2">
                <h3 className="text-[10px] font-semibold text-gray-800 mb-1">3.4 Atividades por Área de Atuação</h3>
                <table className="w-full border-collapse border border-gray-300 text-[8px]">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="border border-gray-300 px-1 py-0.5 text-left">Área</th>
        ...
        {showClientes && clientesData && (
          <section className="mb-3 page-break-before">
            ...
            <div className="grid grid-cols-4 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center">
                <p className="text-base font-bold text-blue-600">{clientesData.processosPorCliente?.length || 0}</p>
            ...
            {clientesData.processosPorCliente?.length > 0 && (
              <div className="mb-2">
                <h3 className="text-[10px] font-semibold text-gray-800 mb-1">4.3 Processos por Cliente (Top 20)</h3>
                <table className="w-full border-collapse border border-gray-300 text-[8px]" data-pdf-allow-split>
                  <thead>
                    <tr className="bg-gray-100">
                  ...
                  <tbody>
                    {clientesData.processosPorCliente.slice(0, 20).map((cliente: any) => (
                      <tr key={cliente.nome}>
                        <td className="border border-gray-300 px-1 py-0.5 truncate max-w-[120px]">{cliente.nome}</td>
        ...
        {showClientes && clientesData && (
          <section className="mb-3 page-break-before">
            ...
            {clientesData.duracaoClientes?.length > 0 && (
              <div className="mb-3">
                <h3 className="text-[10px] font-semibold text-gray-800 mb-1">4.4 Duração Média dos Processos por Cliente (Top 15)</h3>
                <table className="w-full border-collapse border border-gray-300 text-[8px]">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="border border-gray-300 px-1 py-0.5 text-left">Cliente</th>
            ...
            {clientesData.atividadesPorTarefa?.length > 0 && (
              <div className="mb-3">
                <h3 className="text-[10px] font-semibold text-gray-800 mb-1">4.5 Atividades por Tipo de Tarefa</h3>
                <table className="w-full border-collapse border border-gray-300 text-[8px]">
                  <thead>
                    <tr className="bg-gray-100">
        ...
        {/* ================== PÁGINA 5-6: COORDENAÇÕES ================== */}
        {showCompleto && completoData?.coordenacoes && completoData.coordenacoes.length > 0 && (
          <section className="mb-3 page-break-before">
            <h2 className="text-sm font-bold text-gray-900 border-b border-teal-600 pb-1 mb-2">
              5. ESTRUTURA DAS COORDENAÇÕES
            </h2>

            <div className="grid grid-cols-3 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-teal-600">{completoData.coordenacoes.length}</p>
                <p className="text-[8px] text-gray-600">Coordenações</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
            ...
            <div className="mb-3">
              <h3 className="text-[10px] font-semibold text-gray-800 mb-1">5.1 Resumo por Coordenação</h3>
              <table className="w-full border-collapse border border-gray-300 text-[8px]">
                <thead>
                  <tr className="bg-gray-100">
            ...
            {/* 5.2 Processos por Membro - Cards compactos em grid 3 colunas */}
            <div className="mb-2">
              <h3 className="text-[10px] font-semibold text-gray-800 mb-1">5.2 Processos por Membro</h3>
              <div className="grid grid-cols-3 gap-2">
                {completoData.coordenacoes.map((coord) => (
                  coord.membros.length > 0 && (
                    <div key={coord.id} className="border border-gray-200 rounded p-1 bg-gray-50">
                      <h4 className="text-[9px] font-medium text-teal-700 mb-0.5 border-b border-teal-200 pb-0.5">
                        {coord.nome} ({coord.membros.length})
                      </h4>
                      <div className="space-y-0">
                        {coord.membros.slice(0, 8).map((m, i) => (
                          <div key={i} className="flex justify-between text-[8px]">
                            <span className="text-gray-700 truncate" style={{ maxWidth: '80%' }}>{m.nome}</span>
                            <span className="font-semibold text-gray-800">{m.processos}</span>
                          </div>
                        ))}
                        {coord.membros.length > 8 && (
                          <div className="text-[7px] text-gray-500 italic">+{coord.membros.length - 8} membros...</div>
                        )}
                      </div>
                    </div>
                  )
        ...
        {showCompleto && completoData?.audienciasStats && (
          <section className="mb-3 page-break-before">
            <h2 className="text-sm font-bold text-gray-900 border-b border-orange-600 pb-1 mb-2">
              6. PAINEL DE AUDIÊNCIAS
            </h2>

            <div className="grid grid-cols-6 gap-1 mb-3">
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-gray-800">{completoData.audienciasStats.total}</p>
                <p className="text-[7px] text-gray-600">Total</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-amber-600">{completoData.audienciasStats.pendentes}</p>
                <p className="text-[7px] text-gray-600">Pendentes</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-red-600">{completoData.audienciasStats.proximas7Dias}</p>
                <p className="text-[7px] text-gray-600">Próx. 7 Dias</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-green-600">{completoData.audienciasStats.confirmadas}</p>
                <p className="text-[7px] text-gray-600">Confirmadas</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-blue-600">{completoData.audienciasStats.tratadas}</p>
                <p className="text-[7px] text-gray-600">Tratadas</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-gray-500">{completoData.audienciasStats.ignoradas}</p>
        ...
        {showCompleto && completoData?.intimacoesStats && (
          <section className="mb-3">
            <h2 className="text-sm font-bold text-gray-900 border-b border-rose-600 pb-1 mb-2">
              7. PAINEL DE INTIMAÇÕES
            </h2>

            <div className="grid grid-cols-6 gap-1 mb-3">
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-gray-800">{completoData.intimacoesStats.total}</p>
                <p className="text-[7px] text-gray-600">Total</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-amber-600">{completoData.intimacoesStats.pendentes}</p>
                <p className="text-[7px] text-gray-600">Pendentes</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-red-600">{completoData.intimacoesStats.vencidas}</p>
                <p className="text-[7px] text-gray-600">Vencidas</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-orange-600">{completoData.intimacoesStats.proximas7Dias}</p>
                <p className="text-[7px] text-gray-600">Próx. 7 Dias</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-blue-600">{completoData.intimacoesStats.emAndamento}</p>
                <p className="text-[7px] text-gray-600">Em Andamento</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-sm font-bold text-green-600">{completoData.intimacoesStats.tratadas}</p>
                <p className="text-[7px] text-gray-600">Tratadas</p>
              </div>
            </div>
          </section>
        ...
        {showCompleto && completoData?.djenStats && (
          <section className="mb-3">
            <h2 className="text-sm font-bold text-gray-900 border-b border-cyan-600 pb-1 mb-2">
              8. ANÁLISE DJEN (PUBLICAÇÕES)
            </h2>

            <div className="grid grid-cols-3 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-gray-800">{completoData.djenStats.totalPublicacoes}</p>
                <p className="text-[8px] text-gray-600">Total Publicações</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-amber-600">{completoData.djenStats.publicacoesNaoLidas}</p>
                <p className="text-[8px] text-gray-600">Não Lidas</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-green-600">{completoData.djenStats.publicacoesHoje}</p>
            ...
            {completoData.djenStats.porCoordenacao.length > 0 && (
              <div className="mb-2">
                <h3 className="text-[10px] font-semibold text-gray-800 mb-1">8.1 Publicações por Coordenação</h3>
                <table className="w-full border-collapse border border-gray-300 text-[8px]">
                  <thead>
                    <tr className="bg-gray-100">
                      <th className="border border-gray-300 px-1 py-0.5 text-left">Coordenação</th>
        ...
        {showCompleto && completoData?.notificacoesStats && (
          <section className="mb-3">
            <h2 className="text-sm font-bold text-gray-900 border-b border-pink-600 pb-1 mb-2">
              9. CENTRAL DE NOTIFICAÇÕES
            </h2>

            <div className="grid grid-cols-4 gap-1 mb-2">
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-gray-800">{completoData.notificacoesStats.total}</p>
                <p className="text-[8px] text-gray-600">Total</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-amber-600">{completoData.notificacoesStats.naoLidas}</p>
                <p className="text-[8px] text-gray-600">Não Lidas</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-red-600">{completoData.notificacoesStats.prazosUrgentes}</p>
                <p className="text-[8px] text-gray-600">Prazos Urgentes</p>
              </div>
              <div className="border border-gray-300 rounded p-1 text-center bg-white">
                <p className="text-base font-bold text-blue-600">{completoData.notificacoesStats.alertasSistema}</p>
        ...
        {/* Rodapé */}
        <footer className="mt-4 pt-2 border-t border-gray-300 text-center text-[8px] text-gray-500">
