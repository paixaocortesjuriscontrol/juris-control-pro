...
export function BuscaDjenDiretaCard({ monitoramentosIds, className }: BuscaDjenDiretaCardProps) {
  const { progresso, executando, executarMonitoramento, cancelarExecucao } = useBuscaDjenDireta();
        ...
        <div className="flex items-center justify-between">
          ...
          {progresso.status !== 'idle' && (
            <Badge variant="secondary" className={getStatusColor()}>
              {progresso.status === 'executando' ? 'Executando...' : 
               progresso.status === 'concluido' ? 'Concluído' : 
               progresso.status === 'erro' ? 'Erro' : 'Aguardando'}
            </Badge>
          )}
        </div>
      </CardHeader>
        ...
        <div className="flex gap-2">
          {executando ? (
            ...
            <Button 
              variant="default" 
              size="sm" 
              onClick={() => executarMonitoramento(monitoramentosIds)}
              className="w-full"
            >
