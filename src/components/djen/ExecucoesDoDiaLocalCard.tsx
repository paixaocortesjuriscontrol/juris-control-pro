...
import {
  useExecucoesDoDiaLocal,
  type ExecucaoLocalDoDia,
  type TipoEngineLocal,
} from "@/hooks/useExecucoesDoDiaLocal";

interface Props {
  coordenacaoId: string | null | undefined;
  dataDisponibilizacao: string | null | undefined;
  execucaoSelecionadaId: string | null;
  onSelecionarExecucao: (exec: ExecucaoLocalDoDia | null) => void;
}
...
export function ExecucoesDoDiaLocalCard({
  coordenacaoId,
  dataDisponibilizacao,
  execucaoSelecionadaId,
  onSelecionarExecucao,
}: Props) {
  const { data: execs, isLoading } = useExecucoesDoDiaLocal(
    coordenacaoId,
    dataDisponibilizacao,
  );
  // Colapsado por padrão — o usuário abre clicando no cabeçalho.
