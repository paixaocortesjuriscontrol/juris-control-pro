...
const BuscarDJEN = () => {
  ...
  const toggleExpandAll = () => {
    if (expandedPublicacoes.size === paginatedPublicacoes.length && paginatedPublicacoes.length > 0) {
  ...
  return (
    ...
    >
        ...
        <CardContent>
          {!hasSearched ? (
            ...
            <>
              ...
              {/* Publications list - AnaliseDjen style */}
                      ...
                      <div className="flex items-start gap-2 md:gap-3">
                        ...
                        <div className="flex-1 min-w-0 overflow-hidden">
                          ...
                          <div className="flex flex-wrap items-center gap-1 md:gap-2 mb-1.5 md:mb-2">
                            ...
                            {pub.tribunal && (
                              <Badge variant="outline" className="bg-slate-100 text-slate-700 border-slate-300 text-[10px] md:text-xs px-1.5 md:px-2 py-0 md:py-0.5 max-w-[150px] truncate">
                                {pub.tribunal}
                              </Badge>
                            )}

                            <span className="text-[10px] md:text-xs text-muted-foreground ml-auto flex-shrink-0">
                              {formatDateShort(pub.data)}
                            </span>
                          </div>
                          ...
                          >
                            <div className="flex items-start md:items-center gap-1 md:gap-2 mb-1 flex-wrap">
                              ...
                              {pub.processo ? (
                                <p className="text-xs md:text-sm font-medium text-primary break-all">
                                  Processo {formatProcessoNumero(pub.processo)}
                                </p>
                              ) : (
                            ...
                            <div className="flex flex-wrap items-center gap-2 md:gap-4 text-[10px] md:text-xs ml-4 md:ml-6 mb-1.5">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-amber-600 dark:text-amber-400">{formatDateOnly(pub.data)}</span>
                              </div>
                            </div>

                            {pub.partes && (
                              <p className="text-[10px] md:text-xs text-muted-foreground mb-1 ml-4 md:ml-6 break-words">
                                <strong>Partes:</strong> {pub.partes}
                              </p>
                            )}
                          ...
                          {/* Expanded content - formato oficial DJEN */}
                          {isExpanded && (
                            <div className="mt-2 md:mt-3 border-t pt-2 md:pt-3">
                              <PublicacaoConteudoDjen
                                processoNumero={pub.processo || null}
                                tribunal={pub.tribunal || null}
                                fonte={pub.tribunal || null}
                                dataDisponibilizacao={pub.data || null}
                                dataPublicacao={pub.data || null}
                                conteudo={pub.conteudo || null}
                                poloAtivo={pub.partes || null}
                                poloPassivo={null}
      ...
      {/* View Content Dialog - formato oficial DJEN */}
          ...
          <DialogHeader className="pb-2">
            ...
            <DialogDescription className="flex flex-wrap items-center gap-2 text-xs md:text-sm">
              {selectedPublicacao?.processo && (
                <span className="font-mono">{formatProcessoNumero(selectedPublicacao.processo)}</span>
              )}
