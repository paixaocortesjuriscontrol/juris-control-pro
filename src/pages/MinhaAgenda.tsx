          ...
          <div className="flex items-start justify-between gap-2">
            <div className="space-y-1 min-w-0 flex-1">
              ...
              <h3 className={cn(
                ...
                item.status === "cancelado" && "line-through text-muted-foreground"
              )}>
                {item.titulo || "Sem título"}
                {itensComAtividades.has(getItemRawId(item.id)) && <AtividadeBadge className="w-3.5 h-3.5 text-[8px] shrink-0" />}
                {temComentarioItem(itensComComentarios, item) && <ComentarioBadge className="w-3.5 h-3.5 text-[8px] shrink-0" autoria={autoriaComentarioItem(itensComComentarios, item)} />}
              </h3>
            </div>
