...
const AnaliseDjen = () => {
   ...
   * "Ante o exposto, NEGO PROVIMENTO...", com Brasília + Ministro Relator) NUNCA
   * pode faltar no resumo. Se, por qualquer variação de formatação do DJEN, o
   * dispositivo tiver ficado de fora, ele é reanexado a partir do texto original.
   */
  const garantirConclusao = (resumo: string, textoOriginal: string): string => {
    ...
    // Do marcador até o começo do rodapé do PJe / do bloco de intimados.
    ...
    // Reanexa logo antes do bloco de intimados, quando ele existe.
  ...
  const buildPubMetadata = (pub: any, idx: number, orgaoExtra?: string | null): Paragraph[] => {
    ...
    // Disponibilização e publicação são datas diferentes (o DJEN publica no dia
    // útil seguinte). Mostrar as duas evita a dúvida do tipo "não saiu no dia 28".
  ...
  const allPublicacoes = useMemo(() => {
    ...
    if (tratadas.length > 0 && apenasNaoLidas) {
      // Com o filtro "Não lidas" ativo, a publicação lida nesta sessão sai da lista.
      const lidasIds = new Set(tratadas.filter((p) => p?.lida).map((p) => p.id));
      if (lidasIds.size > 0) result = result.filter((p: any) => !lidasIds.has(p.id));
    } else if (tratadas.length > 0) {
  ...
  // O total dos cards é deduplicado de forma diferente da lista, então uma
  // pequena diferença numérica NÃO significa que faltam publicações.
