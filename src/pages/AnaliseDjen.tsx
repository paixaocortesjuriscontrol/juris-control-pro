...
const AnaliseDjen = () => {
  ...
  // ===== "Gerar Docs TST" - Classifica publicações e gera 3 documentos Word =====
  const handleGerarDocsTST = async () => {
    if (allPublicacoes.length === 0) {
      toast.error("Nenhuma publicação para classificar");
      return;
    }

    setGerandoDocsTST(true);
    const totalPubs = allPublicacoes.length;
    const toastId = toast.loading(`Classificando ${totalPubs} publicações com IA...`);

    try {
      // 1. Chamar edge function para classificar
      const pubsPayload = allPublicacoes.map(p => ({
        id: p.id,
      ...
      const { data: classData, error: classError } = await supabase.functions.invoke('classificar-publicacoes-tst', {
        body: { publicacoes: pubsPayload },
      });

      if (classError) throw classError;
      if (!classData?.classificacoes) throw new Error("Classificação não retornada pela IA");

      const classificacoes = classData.classificacoes as Array<{
        id: string;
      ...
      // Mapear classificações por ID
      const classMap = new Map(classificacoes.map(c => [c.id, c]));

      // 2. Separar publicações nas 3 categorias
      const pubsTemasIRR: Array<{ pub: typeof allPublicacoes[0]; class_info: typeof classificacoes[0] }> = [];
      const pubsPauta: Array<{ pub: typeof allPublicacoes[0]; class_info: typeof classificacoes[0] }> = [];
      const pubsPrazos: Array<{ pub: typeof allPublicacoes[0]; class_info: typeof classificacoes[0] }> = [];

      allPublicacoes.forEach(pub => {
        const classInfo = classMap.get(pub.id) || { id: pub.id, categoria: "PRAZOS" as const };
        if (classInfo.categoria === "TEMAS_IRR") pubsTemasIRR.push({ pub, class_info: classInfo });
        else if (classInfo.categoria === "PAUTA") pubsPauta.push({ pub, class_info: classInfo });
        else pubsPrazos.push({ pub, class_info: classInfo });
      });

      toast.loading(`Gerando documentos Word... (Temas: ${pubsTemasIRR.length}, Pauta: ${pubsPauta.length}, Prazos: ${pubsPrazos.length})`, { id: toastId });

      // 3. Gerar os 3 documentos Word
      const dataStr = format(new Date(), "dd.MM.yy");

      const buildTSTDocChildren = (
        pubs: Array<{ pub: typeof allPublicacoes[0]; class_info: typeof classificacoes[0] }>,
        titulo: string
      ): Paragraph[] => {
        const children: Paragraph[] = [...buildDocHeader(titulo, pubs.length)];

        pubs.forEach((item, idx) => {
          const { pub, class_info } = item;

          // Cabeçalho da publicação - estilo COMUNICAÇÃO PJE
          children.push(new Paragraph({
            alignment: AlignmentType.LEFT,
            spacing: { before: idx > 0 ? 360 : 120, after: 120 },
            shading: { type: ShadingType.SOLID, color: mediumBlue, fill: mediumBlue },
            children: [
              new TextRun({ text: `  COMUNICAÇÃO PJE #${sanitizeForXml(pub.processo_numero || "N/A")}`, bold: true, size: 24, color: "FFFFFF", font: docFont }),
            ],
          }));

          // Metadados
          const metaLines = [
            `Processo: ${sanitizeForXml(pub.processo_numero || "N/A")}`,
            `Órgão: ${sanitizeForXml(pub.orgao || pub.tribunal || "N/A")}`,
            `Data de disponibilização: ${pub.data_disponibilizacao ? formatDateOnlyFull(pub.data_disponibilizacao) : "N/A"}`,
            `Tipo de Comunicação: ${sanitizeForXml(pub.tipo_comunicacao || "Intimação")}`,
            `Meio: D`,
          ];
          ...
          metaLines.forEach(line => {
            children.push(new Paragraph({
              spacing: { after: 40 },
              children: [new TextRun({ text: line, size: docFontSize, font: docFont, color: "333333" })],
            }));
          });

          // Observação da IA (se tiver tema IRR)
          if (class_info.tema_irr) {
            children.push(new Paragraph({
              spacing: { before: 80, after: 80 },
              shading: { type: ShadingType.SOLID, color: "FFF3CD", fill: "FFF3CD" },
              children: [
                new TextRun({ text: `  ⚠ Tema IRR: ${sanitizeForXml(class_info.tema_irr)}`, bold: true, size: docFontSize, font: docFont, color: "856404" }),
              ],
            }));
          }

          if (class_info.observacao_ia) {
            children.push(new Paragraph({
              spacing: { before: 60, after: 80 },
              indent: { left: 180 },
              children: [
                new TextRun({ text: "IA: ", bold: true, size: 18, font: docFont, color: "6B7280", italics: true }),
                new TextRun({ text: sanitizeForXml(class_info.observacao_ia), size: 18, font: docFont, color: "6B7280", italics: true }),
              ],
            }));
          }
      ...
      const createWordDoc = (children: Paragraph[]) => {
        return new Document({
          styles: {
            default: {
              document: {
      ...
      // Gerar e baixar cada documento apenas se tiver publicações
      let docsGerados = 0;

      if (pubsTemasIRR.length > 0) {
        const temasDoc = createWordDoc(buildTSTDocChildren(pubsTemasIRR, `Temas IRR - Publicações ${dataStr}`));
        await downloadDoc(temasDoc, `TEMAS_IRR_${dataStr}.docx`);
        docsGerados++;
      }

      if (pubsPauta.length > 0) {
        const pautaDoc = createWordDoc(buildTSTDocChildren(pubsPauta, `Pauta de Julgamento - Publicações ${dataStr}`));
        await downloadDoc(pautaDoc, `PAUTA_${dataStr}.docx`);
        docsGerados++;
      }

      if (pubsPrazos.length > 0) {
        const prazosDoc = createWordDoc(buildTSTDocChildren(pubsPrazos, `Prazos e Decisões - Publicações ${dataStr}`));
        await downloadDoc(prazosDoc, `PRAZOS_${dataStr}.docx`);
        docsGerados++;
      }

      toast.success(
        `${docsGerados} documento(s) gerado(s) com sucesso! (Temas: ${pubsTemasIRR.length}, Pauta: ${pubsPauta.length}, Prazos: ${pubsPrazos.length})`,
        { id: toastId }
      );
    } catch (error) {
      console.error("Erro ao gerar Docs TST:", error);
      const msg = error instanceof Error ? error.message : "Erro desconhecido";
      toast.error(`Erro ao gerar Docs TST: ${msg}`, { id: toastId });
    } finally {
      setGerandoDocsTST(false);
    }
  };
