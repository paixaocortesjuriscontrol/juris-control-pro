...
export default function AdminTstBasePcaDistribuicoes() {
  ...
  const COLUNAS_MODELO = [
    ...
    "Tribunal",
    "Tipo de Recurso",
    "Data da Distribuição no TST STF",
    "Recorrente",
    "Turma",
    "Relator",
    "Recorrente Pos Turma Objeto Recurso Favorável",
    "Recorrente Pos Turma Objeto Recurso Desfavorável",
    "Posicionamento Relator Objeto Recurso Favorável",
    "Posicionamento Relator Objeto Recurso Desfavorável",
    "Recorrente Recurso Bem Aparelhado",
    "Recorrente Recurso Mal Aparelhado",
    "Recorrente com Chance de Êxito",
    "Recorrente sem Chance de Êxito",
    "Resumo Ganhamos",
    "Resumo Perdemos",
    "Análise do Quarteirizado",
    "Retorno Esclarecimentos dos Quarteirizado",
    "Há Risco de Mídia Negativa",
    "Há Discussão sobre Provas Digitais",
    "Temos Data de Julgamento",
    "Datajulgamento",
    "Horajulgamento",
    "Julgamento",
    "Matéria de Honra",
  ...
  const baixarPlanilhaModelo = () => {
    ...
    const ws = XLSX.utils.aoa_to_sheet([COLUNAS_MODELO, exemplo]);
    ws["!cols"] = COLUNAS_MODELO.map(() => ({ wch: 22 }));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Base PCA");
    XLSX.writeFile(wb, "modelo_base_pca_distribuicoes.xlsx");
  };
