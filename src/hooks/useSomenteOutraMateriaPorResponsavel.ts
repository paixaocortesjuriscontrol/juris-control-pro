...
/**
 * AVISO "Somente Outra Matéria" por responsável: processos prontos cujo quadro
 * de matérias da parte recorrente só tem "Outra Matéria". Não é pendência —
 * apenas um lembrete de conferência (a linha vai para a Carga Benner).
 *
 * Lê a coluna `somente_outra_materia` (gravada ao salvar a ficha, no "Marcar
 * Pronto" em lote e no botão "Verificar Pendências") e distribui os IDs entre
 * os responsáveis.
 */
export function useSomenteOutraMateriaPorResponsavel(filters: DistribuicaoTstFilters) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.warn("[useSomenteOutraMateriaPorResponsavel] falhou:", e);
        if (!cancelled) {
          setMap({});
          setIdsPorUsuario({});
          setIds([]);
