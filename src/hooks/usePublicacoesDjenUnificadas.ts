...
export function usePublicacoesDjenUnificadas(filtros: FiltrosUnificados = {}) {
  ...
  const { data: statsIndependentes, isLoading: isLoadingStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filtros.coordenacaoId && filtros.tipoOrigem !== 'descartada') {
        const allRows: PublicacaoUnificada[] = [];
        ...
        for (let offset = 0; offset < 50000; offset += batchSize) {
          ...
          if (pageError) throw new Error(pageError.message || JSON.stringify(pageError));

          const mapped: PublicacaoUnificada[] = ((pageRows || []) as any[]).map((r) => ({
            id: r.id,
          ...
          allRows.push(...mapped);
          if (mapped.length < batchSize) break;
        }

        const filteredByType = filtros.tipoOrigem === 'termo'
          ? allRows.filter((p) => p.tipo_origem === 'termo')
          : filtros.tipoOrigem === 'parte'
            ? allRows.filter((p) => p.tipo_origem === 'termo' && (p.monitoramento_tipo || '').toLowerCase() === 'parte')
            : filtros.tipoOrigem === 'processo'
              ? allRows.filter((p) => p.tipo_origem === 'processo')
              : allRows;

        const filteredByMonitoramento = filtros.monitoramentoId
          ? filteredByType.filter((p) => p.monitoramento_id === filtros.monitoramentoId)
          : filteredByType;

        const rowsComLeitura = await mergeWithLeituras(user.id, filteredByMonitoramento, !!filtros.apenasNaoLidas);

        return {
          total: rowsComLeitura.length,
          naoLidas: rowsComLeitura.filter((p) => !p.lida).length,
          totalTermos: rowsComLeitura.filter((p) => p.tipo_origem === 'termo').length,
          totalProcessos: rowsComLeitura.filter((p) => p.tipo_origem === 'processo').length,
        };
      }
