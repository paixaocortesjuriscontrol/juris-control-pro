...
export function useDistribuicoesTst(filters: DistribuicaoTstFilters = {}) {
  ...
  const fetchDados = useCallback(async () => {
    ...
    // Processo CNJ: NNNNNNN-DD.AAAA.J.TR.OOOO (20 dígitos)
    const CNJ_LIKE = "_______-__.____._.__.____";
    if (filters.processoStatus === "valido") query = query.like("processo", CNJ_LIKE);
    else if (filters.processoStatus === "invalido") query = query.or(`processo.is.null,processo.eq.,and(processo.not.is.null,processo.neq.,processo.not.like.${CNJ_LIKE})`);
